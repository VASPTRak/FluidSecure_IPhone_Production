...
class Preauth_DepartmentVC: UIViewController,UITextFieldDelegate {
    ...
    var Depatmentdata:NSDictionary!
    ...
    func Departmentdata_data()
    {
        if(defaults.string(forKey: "dateof_DownloadPreAuthDepartmentData") == nil) {
            ...
            if(cf.checkPath(fileName: defaults.string(forKey: "dateof_DownloadPreAuthDepartmentData")!) == true) {
                var getdata = cf.ReadFile(fileName: defaults.string(forKey: "dateof_DownloadPreAuthDepartmentData")!) //Vehicaldetails.sharedInstance.PreAuthVehicleDataFilename)
                ...
                do {
                    Depatmentdata = try JSONSerialization.jsonObject(with: data1, options: JSONSerialization.ReadingOptions.mutableContainers) as? NSDictionary
                ...
                let Message = Depatmentdata["ResponceMessage"] as! NSString
                //let ResponseText = vehicledata["ResponceText"] as! NSString
                if(Message == "success") {
                    
                    let objUserData = Depatmentdata.value(forKey: "DepartmentDataObj") as! NSArray
                    let rowCount = objUserData.count
                    let index: Int = 0
                    for i in 0  ..< rowCount
                    {
                        let JsonRow = objUserData[i] as! NSDictionary
                        let Department_Id = JsonRow.value(forKey: "DepartmentId") as! NSNumber
                        let Department_Name = JsonRow.value(forKey: "DepartmentName") as! NSString
                        let Department_Number = JsonRow.value(forKey: "DepartmentNumber") as! NSString
                        
                        
                        DepartmentId.append("\(Department_Id)" as String)
                        DepartmentName.append("\(Department_Name)" as String)
                        DepartmentNumber.append("\(Department_Number)" as String)
...
//                    if(server_res == "success"){
                    ...
                    for id in 0  ..< self.DepartmentName.count {
                        if(deptno == DepartmentNumber[id]){
                            Vehicaldetails.sharedInstance.deptno = DepartmentNumber[id]
                            is_dept_no = true
                            break;
                        }
                        else
