            ...
            if CLLocationManager.locationServicesEnabled() {
                if #available(iOS 14.0, *) {
                    ...
                    case .notDetermined, .restricted, .denied:
                        print("No access")
                        self.ISLocationpermissiongetfromuser = false
                    case .authorizedAlways, .authorizedWhenInUse:
                        print("Access")
                        self.ISLocationpermissiongetfromuser = true
                    @unknown default:
                        break
                    }
                } else {
...
//        getdatauser()
        ...
        if #available(iOS 14.0, *) {
            let checkpermission = LocalNetworkAuthorization()
        
        checkpermission.requestAuthorization { Bool in
            
        }
        delay(3){
        print(Vehicaldetails.sharedInstance.islocalnetworkpermission!)
        if(Vehicaldetails.sharedInstance.islocalnetworkpermission! == true){}
            else{
...
//            //LocalNetworkAuthorization.netServiceDidPublish(checkpermission)
    ...
    public func requestAuthorization(completion: @escaping (Bool) -> Void) {
        ...
        browser.stateUpdateHandler = { newState in
            ...
            case let .waiting(error):
                ...
                Vehicaldetails.sharedInstance.islocalnetworkpermission = false
                self.reset()
                self.completion?(false)
            default:
                break
        ...
        self.netService = NetService(domain: "local.", type:"_lnp._tcp.", name: "LocalNetworkPrivacy", port: 80)
